...
namespace SmashBros
{
	...
	private:
		...
		// true if the given player's control is currently/previously held, from either the
		// keyboard mapping in "controls", or player (pNum-1)'s gamepad, if one is connected
		static bool isControlDown(byte pNum, byte button);
		static bool wasControlDown(byte pNum, byte button);
