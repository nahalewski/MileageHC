...
namespace SmashBros
{
	...
	// maps a Controls::BUTTON_* to the Gamepad button(s) that should trigger it. Direction
	// buttons come from the d-pad or the left stick; jump is bound to both X and Y and grab to
	// both shoulder buttons, matching the traditional GameCube-style Smash Bros control layout
	bool Controls::isControlDown(byte pNum, byte button)
	{
		int keycode = controls[pNum][button];
		if(keycode!=0 && Game::getKeyPressed(keycode))
		{
			return true;
		}
		...
		{
			case BUTTON_UP:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_UP);
			case BUTTON_DOWN:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_DOWN);
			case BUTTON_LEFT:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_LEFT);
			case BUTTON_RIGHT:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_RIGHT);
			case BUTTON_JUMP:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_X) || Gamepad::getButtonPressed(slot, Gamepad::BUTTON_Y);
			case BUTTON_STANDARD:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_A);
			case BUTTON_SPECIAL:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_B);
			case BUTTON_GRAB:
			return Gamepad::getButtonPressed(slot, Gamepad::BUTTON_LEFTSHOULDER) || Gamepad::getButtonPressed(slot, Gamepad::BUTTON_RIGHTSHOULDER);
			default:
			return false;
		}
	}

	bool Controls::wasControlDown(byte pNum, byte button)
	{
		int keycode = controls[pNum][button];
		if(keycode!=0 && Game::getPrevKeyPressed(keycode))
		{
			return true;
		}
		...
		{
			case BUTTON_UP:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_UP);
			case BUTTON_DOWN:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_DOWN);
			case BUTTON_LEFT:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_LEFT);
			case BUTTON_RIGHT:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_RIGHT);
			case BUTTON_JUMP:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_X) || Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_Y);
			case BUTTON_STANDARD:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_A);
			case BUTTON_SPECIAL:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_B);
			case BUTTON_GRAB:
			return Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_LEFTSHOULDER) || Gamepad::getPrevButtonPressed(slot, Gamepad::BUTTON_RIGHTSHOULDER);
			default:
			return false;
		}
	}
	...
	void Controls::CheckKeysDown()
	{
		for(byte i=1; i<=Global::possPlayers; i++)
		{
			...
			{
				if(isControlDown(i, BUTTON_UP) && !wasControlDown(i, BUTTON_UP))
				{
					buttonUp(i,DOWN);
				}

				if(isControlDown(i, BUTTON_DOWN) && !wasControlDown(i, BUTTON_DOWN))
				{
					buttonDown(i,DOWN);
				}
				
				if(isControlDown(i, BUTTON_LEFT) && !wasControlDown(i, BUTTON_LEFT))
				{
					buttonLeft(i,DOWN);
				}
				
				if(isControlDown(i, BUTTON_RIGHT) && !wasControlDown(i, BUTTON_RIGHT))
				{
					buttonRight(i,DOWN);
				}
				
				if(isControlDown(i, BUTTON_JUMP) && !wasControlDown(i, BUTTON_JUMP))
				{
					buttonX(i,DOWN);
				}
				
				if(isControlDown(i, BUTTON_STANDARD) && !wasControlDown(i, BUTTON_STANDARD))
				{
					buttonA(i,DOWN);
				}
				
				if(isControlDown(i, BUTTON_SPECIAL) && !wasControlDown(i, BUTTON_SPECIAL))
				{
					buttonB(i,DOWN);
				}
				
				if(isControlDown(i, BUTTON_GRAB) && !wasControlDown(i, BUTTON_GRAB))
				{
					buttonZ(i,DOWN);
				}
			}
	...
	void Controls::CheckKeysUp()
	{
		for(byte i=1; i<=Global::possPlayers; i++)
		{
			if(Global::characters[i]!=null && !Global::characters[i]->isCPU() && Global::characters[i]->isAlive())
			{
				...
				{
					buttonUp(i,UP);
					if(isControlDown(i, BUTTON_LEFT))
					{
						Global::characters[i]->buttondir=BUTTONDIR_LEFT;
					}
					else if(isControlDown(i, BUTTON_RIGHT))
					{
						Global::characters[i]->buttondir=BUTTONDIR_RIGHT;
					}
					else if(isControlDown(i, BUTTON_DOWN))
					{
				...
				if(!isControlDown(i, BUTTON_DOWN) && wasControlDown(i, BUTTON_DOWN))
				{
					...
					else if(isControlDown(i, BUTTON_UP))
					{
				...
				{
					buttonLeft(i,UP);
					if(isControlDown(i, BUTTON_RIGHT))
					{
						Global::characters[i]->buttondir=BUTTONDIR_RIGHT;
					}
					else if(isControlDown(i, BUTTON_UP))
