	...
	bool Application::Run(unsigned char orient, bool statusbar)
	{
		if(!gameRunning)
		{
			...
			
			{
				...
				int windowX = 0;
				int windowY = 0;
				// SDL_WINDOW_ALLOW_HIGHDPI is required to get a sharp, correctly-scaled
				// backing store on Retina/high-density phone and tablet screens
				Uint32 windowFlags = SDL_WINDOW_ALLOW_HIGHDPI;
#else
				int windowX = SDL_WINDOWPOS_UNDEFINED;
				int windowY = SDL_WINDOWPOS_UNDEFINED;
				// allow resizing on desktop so the SDL_WINDOWEVENT_SIZE_CHANGED handler can
				// rescale the game to whatever size the user's window/display ends up at
				Uint32 windowFlags = SDL_WINDOW_ALLOW_HIGHDPI | SDL_WINDOW_RESIZABLE;
#endif

				if(borderless)
				{
					window = SDL_CreateWindow(NULL, windowX, windowY, View::windowWidth, View::windowHeight, SDL_WINDOW_OPENGL | SDL_WINDOW_BORDERLESS | SDL_WINDOW_SHOWN | windowFlags);
				}
				else
				{
					window = SDL_CreateWindow(NULL, windowX, windowY, View::windowWidth, View::windowHeight, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | windowFlags);
				}
			}
		...
		while(SDL_PollEvent(&event))
		{
			switch(event.type)
			{
				...
				switch(event.window.event)
				{
					...
					// RESIZED only fires for user/window-manager driven resizes; SIZE_CHANGED
					// also covers programmatic resizes and display/DPI changes (e.g. rotating
					// a device or dragging a window to another monitor), so handle both the
					// same way to keep View's scale/letterbox math in sync with the real window
				...
				Gamepad::deviceAdded(event.cdevice.which);
				break;

				case SDL_CONTROLLERDEVICEREMOVED:
				Gamepad::deviceRemoved(event.cdevice.which);
				break;

				case SDL_CONTROLLERBUTTONDOWN:
				Gamepad::buttonEvent(event.cbutton.which, event.cbutton.button, true);
				break;

				case SDL_CONTROLLERBUTTONUP:
				Gamepad::buttonEvent(event.cbutton.which, event.cbutton.button, false);
				break;

				case SDL_CONTROLLERAXISMOTION:
				Gamepad::axisEvent(event.caxis.which, event.caxis.axis, event.caxis.value);
				break;
			}
        }
		SDL_JoystickUpdate();
